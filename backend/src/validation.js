...
export const record = z
  .string()
  .regex(/^\d{5,20}$/, "El registro debe contener entre 5 y 20 dígitos.");
...
export const post = z
  ...
  .refine(
    (v) => Boolean(v.course_id) !== Boolean(v.teacher_id),
    "Selecciona exactamente un curso o un docente.",
  );
export const comment = z.strictObject({
  message: z.string().trim().min(1).max(2000),
});
export const approved = z.strictObject({
