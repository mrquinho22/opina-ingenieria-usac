...
export class Auth {
  ...
  async load() {
    const u = await this.api.get<User>("/auth/me");
    this.user.set(u);
    return u;
  }
  async login(body: unknown) {
    this.user.set(await this.api.post<User>("/auth/login", body));
  }
  async logout() {
...
export function errorMessage(e: unknown) {
  if (e instanceof HttpErrorResponse) {
    ...
    const details = e.error?.errors
      ...
      .join(" ");
    return details || e.error?.message || "No se pudo completar la operación.";
  }
